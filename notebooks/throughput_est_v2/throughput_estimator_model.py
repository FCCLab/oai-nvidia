...
class ThroughputEstimatorBase(nn.Module):
    ...
    def test_with_preload_data(self, preloaded_test_data):
        ...
        with torch.no_grad():  # Disable gradient computation
            for i, (batch_mac, batch_fapi, batch_iq, batch_y_noise_level, batch_y_target_throughput_mbps, batch_y_max_throughput_mbps) in enumerate(preloaded_test_data):
                ...
                p_noise_level.append(batch_y_noise_level)
                p_target_throughput_mbps.append(batch_y_target_throughput_mbps)
        ...
        rmse_noise_level = np.sqrt(mean_squared_error(y_noise_level, p_noise_level))
        ...
        rmse_target_throughput_mbps = np.sqrt(mean_squared_error(y_target_throughput_mbps, p_target_throughput_mbps))
    ...
    def show_test(self, preloaded_test_data, step=1, start_idx=None, stop_idx=None):
        ...
        axs[2].scatter(time_steps, p_sub_max_throughput_mbps, label='Estimated Max Throughput', color='red', alpha=0.7, s=10)
...
class ThroughputEstimatorMacIq(ThroughputEstimatorBase):
    ...
    def __init__(self, lstm_input_size, output_size, hidden_size, num_layers, drop_out=0.1):
        super().__init__(lstm_input_size, output_size, hidden_size, num_layers, drop_out)
